...
- **T-007 — PARCIAL:** seções continuam concluindo automaticamente como `SECTIONS_ONLY`; síntese global permanece POST explícito. `groq_synthesis.py` implementa `GlobalSynthesisProvider` com prompt/schema/orçamento/evidence policy próprios e só aceita citações a segmentos cujo texto foi enviado. `serve.py` agora permite `--synthesis-provider groq` separado de `--analysis-provider`; omissão mantém síntese desligada. SHA `1d78f93a52fbef6817374fd10ac7692d0901bcd5`, Actions 36245879695 PASS Python 3.12/3.13, **227 testes** no log 3.13. Nenhuma chamada autenticada ou avaliação de qualidade real foi feita.
...
T-006 continua bloqueada para identidade/autorização/deep links sem fonte externa verificável. T-007 agora tem adaptador Groq de síntese e composição independente validados offline; o próximo risco útil não é “mais um adaptador”, e sim **validar o percurso completo de seleção/configuração sem realizar chamada remota**, incluindo o smoke/consentimento da síntese e a diferenciação explícita entre análise de seções e segunda operação de síntese. Se isso já estiver coberto ao revalidar o HEAD, escolher uma pendência isolável de T-005/T-007. Não executar Groq real, não criar fallback e não alterar merge/deploy automaticamente.
