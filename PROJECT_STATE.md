...
- **T-007 — PARCIAL:** seções continuam concluindo como `SECTIONS_ONLY`, mas agora existem `GlobalSynthesisProvider`, checkpoint global versionado e `POST /api/jobs/{id}/synthesis` explícito. A revisão `6016d041beb7bdbcbe12533ec86683016d36479d` passou no Actions 36244624678 em Python 3.12/3.13 (212 testes no log 3.13). O endpoint não é disparado pelo worker, exige seções completas e reutiliza checkpoint na repetição. O `serve.py` ainda não registra provedor real de síntese; validação semântica/qualidade e E2E continuam pendentes.
...
T-006 continua bloqueada para promoção de identidade/autorização/deep links sem fonte externa verificável. T-007 agora tem uma fronteira HTTP neutra e explícita; a próxima fatia segura é implementar **um adaptador real de síntese atrás de `GlobalSynthesisProvider`** ou, se isso exigir chamada/custo real para validação, manter o adaptador testável offline e registrar a seleção separada no composition root. Não reutilizar automaticamente `AnalysisProvider` como síntese, não disparar síntese no worker e não criar fallback silencioso. Revalidar PR/HEAD antes de escrever. Sem merge/deploy automático.
