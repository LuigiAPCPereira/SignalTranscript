...
- **T-007 — PARCIAL:** além do adaptador/checkpoint/POST explícito, o smoke agora trata análise por seções e síntese como duas operações independentes. Todos os consentimentos são validados antes do primeiro POST; antes da síntese o cliente revalida `synthesis_provider/model`, e a cobertura global é recalculada localmente. SHA `93168c1d6a5038d6e5f70775500f6db5a9b80581`, Actions 36253734809 PASS Python 3.12/3.13, **232 testes**. Ainda não há chamada autenticada, avaliação de qualidade ou GET dedicado do resultado global.
...
A diferenciação e o consentimento das duas operações estão cobertos offline. A próxima lacuna operacional de T-005/T-007 é recuperação após timeout da síntese: existe checkpoint global, mas o cliente não consegue **consultá-lo sem repetir o POST potencialmente remoto**. A próxima fatia segura deve adicionar uma leitura `GET /api/jobs/{id}/synthesis` (ou status equivalente) que devolva somente checkpoint já persistido e nunca dispare IA, seguida de suporte do smoke para “consultar antes de tentar novamente”. Não executar Groq real, não criar retry/fallback automático e não alterar merge/deploy.
